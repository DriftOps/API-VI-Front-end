        ...
        <div class="active-goal-section card-style">
          ...
          <div v-if="userPlanGoal && userPlanGoal.goalWeight" class="goal-progress-bar">
            ...
            <div class="progress-labels">
              ...
              <span class="current-weight-label" :style="{ left: goalProgressPercentage + '%' }"
                v-if="typeof currentWeight === 'number'">
                {{ currentWeight.toFixed(1) }}kg
              </span>

              <span>{{ userPlanGoal.goalWeight }}kg</span>
            </div>
            <p class="progress-percentage" v-if="typeof goalProgressPercentage === 'number'">{{
              goalProgressPercentage.toFixed(0) }}% concluído</p>
        ...
        <div class="progress-section">
          ...
          <div class="progress-grid">
            ...
            <div class="progress-card card-style weight-log-card">
              ...
              <div v-if="weightChange !== 0 && typeof weightChange === 'number'" class="sub-value change"
            ...
            <div v-if="userPlanGoal && userPlanGoal.goalWeight" class="progress-card card-style goal-comparison-card">
              <h3>Rumo à Meta</h3>
              <div class="main-value" v-if="typeof weightDeltaGoal === 'number'"
                :class="{ positive: weightDeltaGoal <= 0 && userPlanGoal.goalType === 'PERDER_PESO' || weightDeltaGoal >= 0 && userPlanGoal.goalType === 'GANHAR_PESO', negative: weightDeltaGoal > 0 && userPlanGoal.goalType === 'PERDER_PESO' || weightDeltaGoal < 0 && userPlanGoal.goalType === 'GANHAR_PESO' }">
                {{ Math.abs(weightDeltaGoal).toFixed(1) }}kg
              </div>
              <div v-else class="main-value">-- kg</div>
              <div class="sub-value">
                {{ weightDeltaGoal === 0 ? 'Meta atingida!' : ((userPlanGoal.goalType === 'PERDER_PESO' &&
                  weightDeltaGoal < 0) || (userPlanGoal.goalType === 'GANHAR_PESO' && weightDeltaGoal > 0) ? 'restantes' : (
                  (userPlanGoal.goalType === 'PERDER_PESO' && weightDeltaGoal > 0) || (userPlanGoal.goalType ===
                    'GANHAR_PESO' && weightDeltaGoal < 0) ? 'além da meta' : '')) }} </div>
                    <p class="faint info-text">
        ...
        <div v-if="showWeightLogModal" class="modal-overlay" @click.self="showWeightLogModal = false">
          <div class="modal-content">
            ...
            <div class="input-group">
              ...
              <input type="number" id="newWeightModalInput" v-model.number="newWeightEntry" placeholder="Ex: 75.5"
                step="0.1" />
            </div>
          ...
          <div class="modal-content goal-setter-modal-content">
            ...
            <form @submit.prevent="saveGoal">
              <div class="input-group">
                ...
                <input type="number" id="goalWeightInputModal" v-model.number="goalForm.goalWeight"
                  placeholder="Ex: 75.0" step="0.1" required>
              </div>
              <div class="input-group">
...
.sub-value.change .faint {
  font-size: 0.8em;
  margin-left: 5px;
}

.sub-value.change.positive {
  color: #10b981;
}

.sub-value.change.negative {
  color: #ef4444;
}
...
.btn-primary {
  background-color: var(--primary-color);
  color: white;
  box-shadow: 0 2px 5px rgba(var(--primary-color-rgb, 79, 70, 229), 0.3);
}
...
.btn-primary:not(:disabled):hover {
  background-color: var(--primary-color-dark);
  box-shadow: 0 4px 8px rgba(var(--primary-color-rgb, 79, 70, 229), 0.4);
}

.btn-secondary {
  background-color: var(--button-secondary-bg, var(--card-border));
  color: var(--button-secondary-text, var(--color-text));
}

/* Add variables for better themeing */
.btn-secondary:hover {
  background-color: var(--button-secondary-hover-bg, var(--input-bg));
}

/* --- Estilos específicos para Modal de Meta --- */
.goal-setter-modal-content {
  max-width: 450px;
}
...
:root {
  ...
  /* Usado no hover do redefine-goal-btn? */
  --primary-color-rgb: 79, 70, 229;
  /* Para rgba */
  --button-secondary-bg: var(--card-border);
  /* Cor de fundo botão secundário */
  --button-secondary-text: var(--color-text);
  /* Cor texto botão secundário */
  --button-secondary-hover-bg: var(--input-bg);
  /* Cor hover botão secundário */
