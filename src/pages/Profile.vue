{
  type: uploaded file
  fileName: driftops/api-vi-front-end/API-VI-Front-end-main/src/pages/Profile.vue
  fullContent:
  <template>
    <DashboardLayout>
    ...
    setup() {
      ...
      const loadUserData = async () => {
        try {
          ...
          // Correção: Cast para 'any' para evitar erro de TS se a interface User não tiver 'address'
          // ou se os campos extras não estiverem tipados
          const userAny = userData as any;
  
          form.telefone = userAny.phone || ''
          form.cpf = userAny.cpf || ''
  
          // Carrega o endereço se existir
          if (userAny.address) {
            const addr = userAny.address
            form.endereco = {
      ...
      const saveAvatar = async () => {
        ...
        saving.value = true
        try {
          // TODO: Implementar upload real da imagem para o backend
          console.log('Salvando avatar (simulação):', avatarPreview.value)
          alert('Avatar salvo com sucesso! (Simulação)')
        } catch (err) {
      ...
      // Função que salva os dados pessoais e endereço
      const onSubmit = async () => {
        ...
        try {
          const payload = {
              ...
              address: form.endereco // Envia o endereço formatado
          }
  
          // Correção: Cast para 'any' para permitir envio de campos extras
          // que podem não estar no DTO oficial do frontend
          await updateUser(payload as any)
          await userStore.initUser() // Atualiza a store global
      ...
      const updatePassword = async () => {
        ...
        try {
          ...
          console.log('Atualizando senha (simulação)')
